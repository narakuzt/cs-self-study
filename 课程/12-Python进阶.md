...
| 课程 | csdiy.wiki 数据 |
|---|---|
| [UC Berkeley CS61A](https://cs61a.org) | 3 星，50 小时，先修无；Python、Scheme、SQL；第 4 个 Project 用 Python 实现 Scheme 解释器；讲抽象、函数式编程、数据抽象、面向对象；csdiy.wiki 页标题为 Structure and Interpretation of Computer Programs；teachyourselfcs 推荐 SICP（书）配合 Brian Harvey 讲的 CS61A 视频 |
...
- [ ] 自检：手写一个带参数的装饰器，并说明闭包如何保存状态
- [ ] 自检：用生成器处理大文件，内存占用保持恒定
- [ ] 自检：给项目写 20 条以上 `pytest` 用例，含参数化与夹具
- [ ] （进阶）完成 CS61A 的官方 Project

## 来源与核实

- CS61A 数据引自 csdiy.wiki 原文（MIT 许可证）。
