...
3. 做[知识层](../知识/00-Linux命令行与工具.md)的概念清单与自测题库，对照「过关标准」自测。命令行的艺术、鸟哥、菜鸟教程是可选的速查。
...
4. 做[知识层](../知识/00-Linux命令行与工具.md)的概念清单与自测题库，做错的题加入复习队列。
...
- **0.4** 自检：解释 `chmod 754`；说明直接执行一个脚本需要什么权限，以及 shebang 的作用
- **0.5** 闭卷做完[知识层](../知识/00-Linux命令行与工具.md)的 34 道自测题，做错的加入复习队列，一周后复测
